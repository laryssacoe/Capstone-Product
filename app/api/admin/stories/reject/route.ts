...
export async function PATCH(request: Request) {
  ...
  if (!parsed.success) {
    // Attempt to fall back to query param slug when body missing
  ...
  try {
    ...
    // Ensure story visibility is private after rejection
    ...
    console.error("[api/admin/stories/reject] Failed to reject story", error)
    const message =
      error instanceof Error ? error.message || "Unable to reject story." : "Unable to reject story."
